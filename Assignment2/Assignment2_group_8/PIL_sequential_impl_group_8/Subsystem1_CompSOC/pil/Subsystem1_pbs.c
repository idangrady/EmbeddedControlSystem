 ...
 * Component Simulink Path: Subsystem1
 * Simulation Mode: PIL
 *
 */

#define S_FUNCTION_NAME                Subsystem1_pbs
  ...
  memUnitSizeBytes)
{
  ...
  pCodeInstrServiceData->infoPath =
    "/home/computation/5lij0_ECS_Course_2023/PIL_Implementation/PIL_sequential_impl/Subsystem1_CompSOC/pil";
...
static boolean_T startAndSetupApplication(SimStruct *S)
{
...

  {
...

    {
      ...
      rtIOStreamDataPtr->targetRecvBufferSizeBytes = 1024;
      rtIOStreamDataPtr->targetSendBufferSizeBytes = 1024;
...
static void mdlInitializeSizes(SimStruct *S)
{
  ...
  ssSetInputPortSampleTime(S, 0, 0.03);
  ...
  /* using port based sample times */
  ssSetInputPortSampleTime(S, 1, 0.03);
  ...
  /* using port based sample times */
  ssSetOutputPortSampleTime(S, 0, 0.03);
  ...
  /* using port based sample times */
  ssSetOutputPortSampleTime(S, 1, 0.03);
  ...
  /* using port based sample times */
  ssSetOutputPortSampleTime(S, 2, 0.03);
...
static void XILoutputTID01(SimStruct *S, int tid)
{
...

  {
    ...
    if (IOBufferPtr != NULL) {
      void * pXILService = (void *) ssGetPWorkValue(S, 9);
      if (pXILService != NULL) {
        if (xilHostAppSvcGrowIOBuffer(pXILService, IOBufferPtr, 12509, 0)!=
            XILHOSTAPPSVC_SUCCESS) {
          return;
        }                              /* if */
  ...
  {
    {
      ...
      /* dispatch command to the target */
      if (commandDispatch(S, IOBufferPtr, 12509)!=XILHOSTAPPSVC_SUCCESS) {
        return;
      }                                /* if */
