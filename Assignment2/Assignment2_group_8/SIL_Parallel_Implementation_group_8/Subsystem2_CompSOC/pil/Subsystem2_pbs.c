 ...
 * Component Simulink Path: Subsystem2
 * Simulation Mode: PIL
 *
 */

#define S_FUNCTION_NAME                Subsystem2_pbs
  ...
  memUnitSizeBytes)
{
  ...
  pCodeInstrServiceData->infoPath =
    "/home/computation/5lij0_ECS_Course_2023/SIL_Implementation/SIL_Parallel_Implementation/Subsystem2_CompSOC/pil";
...

  {
...

    {
      mxArray * codeGenComponent = mxCreateString(
        "SIL_Simulation_Parallel_Implementation");
      mxArray *rhs[3];
      const char * simulinkBlockPath = ssGetPath(S);
      rhs[ 0 ] = mxCreateString(
        "@coder.connectivity.SimulinkInterface.getSILPILInterface");
...
static void XILoutputTID01(SimStruct *S, int tid)
{
...

  {
    ...
    if (IOBufferPtr != NULL) {
      void * pXILService = (void *) ssGetPWorkValue(S, 9);
      if (pXILService != NULL) {
        if (xilHostAppSvcGrowIOBuffer(pXILService, IOBufferPtr, 12505, 0)!=
            XILHOSTAPPSVC_SUCCESS) {
          return;
        }                              /* if */
  ...
  {
    {
      ...
      /* dispatch command to the target */
      if (commandDispatch(S, IOBufferPtr, 12505)!=XILHOSTAPPSVC_SUCCESS) {
        return;
      }                                /* if */
