...
#define SEND_FROM_TILE_2_TO_TILE_0
...
void hough_transform()
{
    ...
    float     	data_peaks_tile_2[MAX_PEAK_NUMBER] = {0};
    float       diff_Err;
    float       control_input[1] = {0};
    uint8_t 	picture_lock = read_lock(PICTURE_LOCK_LOCATION_MEM_02);
    if (picture_lock == LOCK)
    {
        read_byte (rcv_img_data, PICTURE_LOCATION_MEM_02, IMG_SIZE);
            ...
            for (int y = 0; y < IMG_HEIGHT; y++) { // 142
                if (rcv_img_data[y + IMG_HEIGHT * x]==1) {
                    for (int thetaIdx = 0; thetaIdx < NUMBER_OF_THETA; thetaIdx++) {  // Loop from THETA_MIN to THETA_MAX
                        int16_t rho  =   lroundf(x * cost[thetaIdx] +  y * sint[thetaIdx] + MAX_NEGATIVE_RHO); // + IMG_HEIGHT*sind(THETA_MIN)
        ...
        for (int a = 0; a < MAX_PEAK_NUMBER; a++)
        {
            ...
            data_peaks_tile_2[a] = distance;
        }
        
        
        // Calculate center point
        current_pos[0] = (int)(data_peaks_tile_2[0] + data_peaks_tile_2[1]) / 2.0;
        write_float(current_pos, CURRENT_POS_LOCATION_MEM_02, 1);
        ...
        write_float(control_input, FORCE_LOCATION_MEM_02, 1);
        write_lock(UNLOCK, PICTURE_LOCK_LOCATION_MEM_02);
        write_lock(LOCK, FORCE_LOCK_LOCATION_MEM_02);
